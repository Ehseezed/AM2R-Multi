...
class FF4FEWorld(World):
        ...
        for location in self.multiworld.get_filled_locations(self.player):
            ...
            if location.item.player == self.player:
                ...
                placement_dict[location_data.fe_id] = {
                    ...
                    "item_name": self.remove_unencodable_characters(location.item.name),
                    "player_name": self.remove_unencodable_characters(self.multiworld.player_name[location.item.player])
                }
    ...
    def remove_unencodable_characters(self, name) -> str: #  added this function to replace unencodable characters with "?"
        # encodeable characters:  '.-_!?%/:,
        chars = "&()<>=+\t\"\'<>~`@#$^*{}[];"  #  I'm sure I missed some but thats why I picked this method
        for c in chars:
            name = name.replace(c, "?")
        return name